class App.Views.Main.Settings.Goals extends App.Views.Base
  ...
  onSaveGoals: =>
    ...
    preference = app.me.get('patientPreferences')
    preference.maintain = @getMaintainPerIndex(slideWeight)
    preference.activity = @getActivityPerIndex(slideActivity)
    preference.workout = @getWorkoutPerIndex(slideWorkout)
    preference.weather = @getWeatherPerIndex(slideWeather)
    ...
    preference.fitness_goals = fitness_goals

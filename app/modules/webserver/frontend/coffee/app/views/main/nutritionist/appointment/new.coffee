class App.Views.Main.Appointment.New extends Null.Views.Base
  ...
  new: () =>
    ...
    appointment.save({}, {
      ...
      error: (model, response) =>
        @$el.unblock()
        @error(response.responseJSON)
        $('#btn-stripe-step span.loader').addClass 'hidden'
      wait: true
    })
  ...
  getStripeToken: (event) =>
    ...
    $('#btn-stripe-step span.loader').removeClass 'hidden'
    $('#btn-stripe-step span.btn-text').hide()
  ...
  createSubscription: (status, stripe) =>
    ...
    if stripe.error
      ...
      $('#btn-stripe-step span.loader').addClass 'hidden'
      $('#btn-stripe-step span.btn-text').show()
      return
